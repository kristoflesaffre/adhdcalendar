...
- **Real alarms** ⏰ — multiple per event, quick chips (5/10/15/20/30/60 min, 1 day) plus custom offsets. A full-screen takeover with a selected alarm sound that loops until dismissed. Snooze 5/10/15 min. Backup system notification when the tab is hidden. Alarms also fire up to 15 min late if the tab was asleep at the trigger moment.
...
- When one is due, the **AlarmOverlay** mounts: pulsing rings, swinging bell, event details, and the selected alarm sound loop. It does not stop until you press **Stop alarm** — or snooze it.
